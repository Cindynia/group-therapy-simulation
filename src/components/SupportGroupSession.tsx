...
import { SupportEvent, ResponseType, generateSupportEvent, generateSessionSummary } from '../services/supportService';
...
interface SupportGroupSessionProps {
  patients: Patient[];
...
const SupportGroupSession: React.FC<SupportGroupSessionProps> = ({ 
  patients, 
  sessionDuration = 45 
}) => {
  ...
  const [messages, setMessages] = useState<Array<SupportEvent | { isTherapist: true, message: string }>>([]);
  ...
  const startSession = () => {
    ...
    const welcomeMessage = {
      ...
      message: "Welcome everyone to today's grief support group. Let's take a moment to check in with how everyone is feeling today."
    };
  ...
  const endSession = () => {
    ...
    // Generate facilitator wrap-up message
    const wrapUpMessage = {
      isTherapist: true as const,
    ...
    const patientEvents = messages.filter(msg => !('isTherapist' in msg)) as SupportEvent[];
  ...
  const schedulePatientResponse = (delay = 3000) => {
    ...
    patientResponseTimeoutRef.current = setTimeout(() => {
      ...
      const event = generateSupportEvent(patients);
      const respondingPatient = patients.find(p => p.id === event.memberId);
  ...
  return (
    <div className="h-full grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
      ...
      {/* Main content - Support group session */}
                  ...
                  <div className="space-y-4">
                    {messages.map((msg, index) => {
                      if ('isTherapist' in msg) {
                        ...
                        const patient = patients.find(p => p.id === msg.memberId);
                        return (
                          <TherapyMessage
                            key={index}
                            content={msg.message}
...
export default SupportGroupSession;
